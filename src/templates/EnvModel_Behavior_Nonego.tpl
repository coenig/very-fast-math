...
@(
   ...
   @{
      ...
      @{
         TRANS veh___6[i]9___.time_since_last_lc < min_time_between_lcs @{| abs(veh___6[i]9___.v) < @{UCD_MIN_VELOCITY_TO_ALLOW_LC}@.velocityWorldToEnvModelConst }@****.if[@{UCD}@.eval] -> veh___6[i]9___.lane_unchanged;
...
veh___6[i]9___.on_lane_min := case
   ...
   }@.for[[j], 0, @{NUM_TECHNICAL_LANES - 2}@.eval]TRUE : @{NUM_TECHNICAL_LANES - 1}@.eval[0];
esac;
...
veh___6[i]9___.on_lane_max := case
   @{veh___6[i]9___.lane_b[j] : [j];
   }@.for[[j], @{NUM_TECHNICAL_LANES - 1}@.eval, 1, -1]TRUE : 0;
esac;
...
}@*.for[[j], 0, @{NUM_TECHNICAL_LANES - 2}@.eval]}@**.for[[i], 0, @{NONEGOS - 1}@.eval].nil

@{@{
   -- veh___6[i]9___.prohibit_lanes_from_[j] := @{!veh___6[i]9___.lane_b[k]}@.for[[k], [j], @{NUM_TECHNICAL_LANES - 1}@.eval, 1, &];
}@*.for[[j], 1, @{NUM_TECHNICAL_LANES - 1}@.eval]}@**.for[[i], 0, @{NONEGOS - 1}@.eval].nil
...
DEFINE
   distance_between_normalized_lanes_cm := @{LANE_WIDTH * (NUMLANES / NUM_TECHNICAL_LANES / 2)}@.eval[0]; -- This is the distance between half-lanes on the "technical" level, i.e., the most fine-granular lateral distance we have.
   ...
   -- Interpolate linearly between MAX_JUMP_UNTIL_VELOCITY and MIN_JUMP_FROM_VELOCITY from MAX_JUMP_OVER_TECHNICAL_LANES to 1.

    max_jump_lanes_[i] := m_j - (veh___6[i]9___.v - vel_min_j) * (m_j - 1) / (vel_min_j - vel_max_j);

    veh___6[i]9___.lane_move_down := 
                      @{(veh___6[i]9___.normalized_lane_[j] -> next(@{(@{[j] - max(0, [k])}@.eval[0] <= max_jump_lanes_[i] & veh___6[i]9___.normalized_lane_@{max(0, [k])}@.eval[0])}@*.for[[k], @{[j] - MAX_JUMP_OVER_TECHNICAL_LANES }@.eval, @{[j] - 1}@.eval, 1, |]))
                      }@**.for[[j], 0, @{normalized_lane_counter}@.scriptVar, 1, &];
    veh___6[i]9___.lane_move_up :=
                      @{(veh___6[i]9___.normalized_lane_[j] -> next(@{(@{min(@{normalized_lane_counter}@.scriptVar, [k]) - [j]}@.eval[0] <= max_jump_lanes_[i] & veh___6[i]9___.normalized_lane_@{min(@{normalized_lane_counter}@.scriptVar, [k])}@.eval[0])}@*.for[[k], @{[j] + 1}@.eval, @{[j] + MAX_JUMP_OVER_TECHNICAL_LANES}@.eval, 1, |]))
                      }@**.for[[j], 0, @{normalized_lane_counter}@.scriptVar, 1, &];
   ...
   @{################# We now change this part again for the "smoother lane changes" feature (2026-03-13) ###################}@.nil
	@{################# FORMERLY:
   @{
...
INVAR -- Non-Ego cars may not collide (except in INIT state, when in UCD mode).
@{cnt = 0 | }@.if[@{UCD}@.eval]
