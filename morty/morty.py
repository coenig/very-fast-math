...
for seedo in range(0, MAX_EXPs):
    ...
    for global_counter in range(args.steps_per_run):
        ...
        for i, el in enumerate(sum_vel_by_car):
            ...
            accel = sum_vel_by_car[i] * 6/3 / ACCEL_RANGE # 5/3 is the factor from EnvModel ==> a_min/a_max to ACCEL_RANGE, a discrepancy accounting for highway-env adjusting acceleration smoothly while the MC only does it once/s.
