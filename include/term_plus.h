...
public:
   ...
   bool isTermPlus() const override;
   std::string getK2Operator() const override;

   TermPlus(const std::vector<std::shared_ptr<Term>>& opds);
   TermPlus(const std::shared_ptr<Term> opd1, const std::shared_ptr<Term> opd2);
};
