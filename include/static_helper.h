...
public:
   ...
   static std::string exec(const std::string cmd); // Deprecated, use the below.
   
   /// Execute the given cmd on system level, and return the output generated by it as well as exit code.
   static std::string execWithSuccessCode(const std::string& cmd, int& exit_code, std::shared_ptr<std::string> path_to_store_meta_info);
